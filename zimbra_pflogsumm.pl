...
Copyright (C) 2025 by Dennis Anfossi, Release 1.1.6
...
my (%recipDom, $recipDomCnt);
my (%sendgDom, $sendgDomCnt);
$msgCntI     = 0;
$msgSizeI    = 1;
$msgDfrsI    = 2;
$msgDlyAvgI = 3;
...
my @dfrPerHr = @rcvPerHr;
my @bncPerHr = @rcvPerHr;
my @rejPerHr = @rcvPerHr;
my $lastMsgDay = 0;
my @smtpdPerHr;
for (0 .. 23) {
    $smtpdPerHr[$_]  = [0,0,0];
...
while(<>) {
    ...
    unless((($cmd, $qid) = $logRmdr =~ m/^(\S+)\/(\S+)\[\d+\]: (?:msgid=)?(\S+): (.*)$/))
    {
        next;
    }
    chomp;
